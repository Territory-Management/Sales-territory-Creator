...
def normalize_numeric_column(value) -> float:
    """Normalize a single value with monetary symbols and non-numeric characters."""
    if pd.isna(value):
        return 0.0
    try:
        # Convert to string safely
        str_value = str(value)
        # Remove monetary symbols and normalize decimal separator
...
def distribute_equally(df: pd.DataFrame, num_territories: int, balance_columns: List[str]) -> List[pd.DataFrame]:
    ...
    for _, row in df.iterrows():
        total = sum(normalize_numeric_column(row[col]) for col in balance_columns)
        row_totals.append(total)
    
    # Create a copy of the dataframe with the totals
    df_with_totals = df.copy()
    df_with_totals['_total'] = row_totals
    
    # Sort by totals
    df_sorted = df_with_totals.sort_values('_total', ascending=False).drop('_total', axis=1)

    # Initial allocation by row count and sum balancing
    sums = [0.0] * num_territories
    counts = [0] * num_territories
    
    for _, row in df_sorted.iterrows():
        # Find territory with minimum sum and count
        min_index = min(range(num_territories), key=lambda i: (counts[i], sums[i]))
        
        # Add row to territory
        territories[min_index] = pd.concat([territories[min_index], pd.DataFrame([row])], ignore_index=True)
        
        # Update counts and sums
        counts[min_index] += 1
        row_sum = sum(normalize_numeric_column(row[col]) for col in balance_columns)
        sums[min_index] += row_sum
