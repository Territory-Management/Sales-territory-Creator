...
def create_balanced_territories(df, num_territories, balance_columns):
    ...
    # Pre-compute cleaned values for efficiency
    ...
    for idx, row in df_sorted.iterrows():
        ...
        best_territory = min(range(num_territories), 
                             key=lambda i: np.sum((territories[i].applymap(clean_numeric_value).sum() + row_values - target_sums) ** 2))
    ...
    if uploaded_file:
        ...
        if st.button("Create Territories"):
            with st.spinner("Distributing territories..."):
                territories = create_balanced_territories(df, num_territories, balance_columns)
